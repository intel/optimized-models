        ...
        for layer in caffe_net.layer:
            ...
            if len(pretrained_layers) > 1:
                ...
                if layer.type == "BatchNorm":
                    pretrained_blobs = []
                    for i, blob in enumerate(pretrained_layers[0].blobs):
                         if i < 3:
                             pretrained_blobs.append(utils.CaffeBlobToNumpyArray(blob))
                         elif i == 3 or i == 4:
                             scale_blobs.append(utils.CaffeBlobToNumpyArray(blob))
                         else:
                             raise ValueError(
                                 'invalid blob numbers for this BatchNorm')
                else:
                    pretrained_blobs = [
                ...
                pretrained_blobs = []
            if scale_blobs and layer.type == "Scale":
                operators, params = cls.TranslateLayer(
                    layer, scale_blobs, is_test, net=net,
...
def TranslateBatchNorm(layer, pretrained_blobs, is_test, **kwargs):
    ...
    log.info("the blob num is {}".format(len(pretrained_blobs)))
...
if __name__ == '__main__':
    ...
    with open(output_init_net + 'txt', 'w') as f:
